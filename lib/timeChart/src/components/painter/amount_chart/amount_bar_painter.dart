...
import '/timeChart/src/components/painter/bar_painter.dart';
  ...
  void drawBar(Canvas canvas, Size size, List<AmountBarItem> coordinates) {
    ...
    for (int index = 0; index < coordinates.length; index++) {
      ...
      callback(_) => tooltipCallback(
            ...
            startTime: offsetWithAmount.startTime,
    ...
    for (int index = startIndex; index < length; index++) {
      ...
      if (index == length - 1 ||
        ...
        coordinates.add(AmountBarItem(
            dx, dy, amountSum, dataList[index].end, dataList[index].start));
...
class AmountBarItem {
  ...
  AmountBarItem(this.dx, this.dy, this.amount, this.dateTime, this.startTime);
}
